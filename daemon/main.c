...
{
	assert(keyfile != NULL);
	auto_free char *cmd = afmt("trust_anchors.config('%s',%s)", keyfile, unmanaged ? "true" : "nil");
	if (!cmd) {
...
int parse_args(int argc, char **argv, struct args *args)
{
	...
	while ((c = getopt_long(argc, argv, "a:t:S:T:c:f:m:K:k:vqVh", opts, &li)) != -1) {
		...
		case 't':
			...
			break;
		case 'S':
			array_push(args->fd_set,  atoi(optarg));
			break;
		case 'T':
			array_push(args->tls_fd_set,  atoi(optarg));
			break;
		case 'c':
			args->config = optarg;
			break;
